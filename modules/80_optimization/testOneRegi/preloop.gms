...
$offtext
        loop(regi,	    
         pm_XportMrkt0(ttot,regi,mrktsPool) = vm_XportMrkt.l(ttot,regi,mrktsPool);
         p80_MportMrkt0(ttot,regi,mrktsPool) = vm_MportMrkt.l(ttot,regi,trade);
       
***in case xport/mport is not found in gdx:
$ontext
        if(pm_XportMrkt0(ttot,regi,mrktsPool) eq NA,
        pm_XportMrkt0(ttot,regi,mrktsPool) = 0;
        vm_XportMrkt.L(ttot,regi,mrktsPool) = 0;
             );
        if(p80_MportMrkt0(ttot,regi,mrktsPool) eq NA,
        p80_MportMrkt0(ttot,regi,mrktsPool) = 0;
        vm_MportMrkt.L(ttot,regi,mrktsPool) = 0;
             );
$offtext
