...
loop(ttot$(ttot.val ge 2005),
    loop(mrktsPool),
        if((pm_pvp(ttot,mrktsPool) eq NA) OR (pm_pvp(ttot,mrktsPool) lt 1E-12) OR (pm_pvp(ttot,mrktsPool) gt 0.1) ,
***in case we have not been able to read price paths from the gdx, or these price are zero (or eps),  fall back to price paths in file input/prices_NASH.inc:
            pm_pvp(ttot,mrktsPool) = p80_pvpFallback(ttot,mrktsPool);
...
***in case pm_pvp is not found in gdx:
        if(pm_pvp(ttot,mrktsPool) eq NA,
        pm_pvp(ttot,mrktsPool) = 0;
         );
        loop(regi,	    
         pm_Xport0(ttot,regi,mrktsPool)  = vm_Xport.l(ttot,regi,mrktsPool);
         p80_Mport0(ttot,regi,mrktsPool) = vm_Mport.l(ttot,regi,mrktsPool);
       
***in case xport/mport is not found in gdx:
        if(pm_Xport0(ttot,regi,mrktsPool) eq NA,
        pm_Xport0(ttot,regi,mrktsPool) = 0;
        vm_Xport.L(ttot,regi,mrktsPool) = 0;
             );
        if(p80_Mport0(ttot,regi,mrktsPool) eq NA,
        p80_Mport0(ttot,regi,mrktsPool) = 0;
        vm_Mport.L(ttot,regi,mrktsPool) = 0;
             );
...
***        p80_normalize0(ttot,regi,"perm") = vm_cons.l(ttot,regi);  
        ...
        p80_normalize0(ttot,regi,mrktsPoolPe) = 0.5 * (sum(rlf, vm_fuExtr.l(ttot,regi,mrktsPoolPe,rlf)) + vm_prodPe.l(ttot,regi,mrktsPoolPe));
...
pm_pvp(t_interpolate,mrktsPool)
= sum(t_input_gdx,
    pm_pvp(t_input_gdx,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );

pm_Xport0(t_interpolate,regi,mrktsPool)
= sum(t_input_gdx,
    pm_Xport0(t_input_gdx,regi,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );

p80_Mport0(t_interpolate,regi,mrktsPool)
= sum(t_input_gdx,
    p80_Mport0(t_input_gdx,regi,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );

vm_Xport.l(t_interpolate,regi,mrktsPool)
= sum(t_input_gdx,
    vm_Xport.l(t_input_gdx,regi,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );

vm_Mport.l(t_interpolate,regi,mrktsPool)
= sum(t_input_gdx,
    vm_Mport.l(t_input_gdx,regi,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );

p80_normalize0(t_interpolate,regi,mrktsPool)
= sum(t_input_gdx,
    p80_normalize0(t_input_gdx,regi,mrktsPool)
  * p80_t_interpolate(t_interpolate,t_input_gdx)
  );
...
loop(regi,
    loop(mrktsPoolPe,
if(p80_Mport0("2005",regi,mrktsPoolPe) eq NA, p80_Mport0("2005",regi,mrktsPoolPe) = 0);
););
...
p80_pvp_itr(ttot,mrktsPool,"1") = pm_pvp(ttot,mrktsPool);
...
loop(mrktsPoolPe,
	...
	if(p80_pvp_itr(ttot,mrktsPoolPe,"1") eq 0,
	    p80_pvp_itr(ttot,mrktsPoolPe,"1") = p80_pvp_itr(ttot-1,mrktsPoolPe,"1")$(NOT sameas(ttot,'2005'));
	    );
    );
);
