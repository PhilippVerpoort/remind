...
pm_Xport0(t,regi,tradePe) = vm_Xport.l(t,regi,tradePe);

*** Secondary energy trade

*** Temporarily forcing Mports (until all bugs are fixed with automatic trading)
$IFTHEN.seTradeConstrained "%cm_seTradeConstrained%" == "on"
...
$ENDIF.seTradeConstrained

*** Xport price
$IFTHEN.seSlowPriceAdjustment "%cm_seSlowPriceAdjustment%" == "on"
if(iteration > 1,
  pm_XPortsPrice(t,regi,tradeSe) = cm_sePriceAdjustmentParam*pm_SEPrice(t,regi,tradeSe) + (1-cm_sePriceAdjustmentParam)*pm_XPortsPrice_iter(iteration,t,regi,tradeSe);
  else
  pm_XPortsPrice(t,regi,tradeSe) = pm_SEPrice(t,regi,tradeSe);
);
$ELSE.seSlowPriceAdjustment
pm_XPortsPrice(t,regi,tradeSe) = pm_SEPrice(t,regi,tradeSe);
$ENDIF.seSlowPriceAdjustment
...
p24_MportsRegi_iter(iteration,t,regi,regi2,tradeSe) = p24_MportsRegi(t,regi,regi2,tradeSe);
p24_XportsRegi_iter(iteration,t,regi,regi2,tradeSe) = p24_XportsRegi(t,regi,regi2,tradeSe);
pm_MPortsPrice_iter(iteration,t,regi,tradeSe) = pm_MPortsPrice(t,regi,tradeSe);
pm_XPortsPrice_iter(iteration,t,regi,tradeSe) = pm_XPortsPrice(t,regi,tradeSe);
pm_SEPriceseel_iter(iteration,t,regi) = pm_SEPrice(t,regi,"seel");
